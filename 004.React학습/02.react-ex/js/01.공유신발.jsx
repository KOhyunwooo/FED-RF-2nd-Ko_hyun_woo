...
function MainComponent() {
    ...
    **************************************/
    //useEffect 테스트 함수////////////////////
    const testFn = () => {
        console.log("테스트중~!test상태 변수값",test);
        //의존성 테스트를 위한 상태변수 업데이트
        setTest(test?false:true);
        //true/false값 상호전환 변경
    }; /////////testFn//////////
    //[[1. useEffect: 의존성이 '없는'경우]]
    //컴포넌트 생성,변경,삭제전 DOM완성후, 매번 실행되는 코드 구역!]
    React.useEffect(() => {
      ...
      React.useEffect(()=>{
        console.log("의존성useEffect실행: seItem")
    ...
    // useState변수중 원하는 변경에 해당하는것을 선택하여
    // 의존성 옵션을 주면 해당변수가 변경될때만 실행하는 
    // 렌더링실행구역이 만들어진다.


    //[3.useEffect:의존성있으나 빈 경우]
    React.useEffect(()=>{
      console.log("useEffect의존성 비우고 한번만 실행!");
     //로고 애니 호출하기////
        comFn.logoAni();
    ...
    return (
        <React.Fragment>
            {/* 1. 타이틀 */}
            <h1 className="tit">
                  <img id="logo" style={{width:"50px",verticalAlign:"-6px",marginRight:"10px"}} src="./images/logo.png" alt="로고" />
                <span>
                  {selItem == "공유"
                      ? "공유가 신고 다닌다는!"
                      : selItem == "효진"
