...
function whoWinner() {
    ...
    if(r1pos>=FINAL_NUM|| t1pos >=FINAL_NUM){
        ...
        //승자변수(메시지 떄문에 씀:토끼/거북/비김)
        let winner;
        //승자판별하기
        if(r1pos>t1pos) winner="토끼";
        else if(r1pos<t1pos) winner="거북이";
        else winner= "비김";
...
/**************************************************
  [내가원하는 랜덤수 만들기]
  1.Math.random()->0~1사이에 소수점 17자리 랜덤수생성
  ->0~1사이 소수점 16자리 랜덤수 생성됨
  2.내가원하는 1~몇까지의 랜덤수 만들기
  (1)Math.random()*원하는 최대수
  ->0~원하는수보다 1작은수까지 랜덤수 발생~
  (2)올림처리 하여 1~원하는수를 만들어준다~
  ->Math.ceil(Math.random()*원하는 최대수)
  ->만약 0 부터 1작은 최대수를 만들고 싶으면
  내림처리하면 된다! Math.floor()
...
//  console.log(Math.ceil(Math.random()*8));
 console.log(Math.ceil(Math.ceil(Math.random()*8)+3));
