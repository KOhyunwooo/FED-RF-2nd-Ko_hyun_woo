...
//->v는 배열값, i 순번,arr 전체 배열
//->>>기본 배열값을 순회하면 변환된 값을 다시 넣어줌!
...
//return키워드를 써줘야함! 따라서 중괄호 생략시 리턴키워드
//자동생략 문법을 사용하면 간단히 표현할 수 있다!

console.log('map변환후 배열값:',newArr);

//배열값을 문자화하여 순회하여 콤보박스에 태그 넣기
...
function showFruit(){
    ...
    if(btxt==='과일주세요~!'){
        ...
        let hcode=`<ul>`;
        fruit.forEach(v=>{
            hcode+=`<li style="background:url(./addimg/${frObj[v]}.png) no-repeat center/cover">
            ${v}
            </li>
            `;
        });////forEach///////////
