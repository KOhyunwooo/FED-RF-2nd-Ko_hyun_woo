...
export default function ItemsArea({ catName }) {
    ...
    return (
        <div id="main-area">
            {/* 데이터 적용1: 최상위 클래스명 추가하기(클래스명 적을때 main-area ibx^ 띄어쓰기 한칸해야함) */}
            <main className={"main-area ibx " + selData.경로}>
                ...
                <header className="cat-top-area">
                    ...
                    {selData.메뉴 != "없음" && ( //메뉴가"없음"이 아닐때만 배열돌아 출력함!(메뉴가 없음이 아니면 출력해라!)
                        <nav className="lnb">
                            <ul>
                                {/* 리액트는 맵돌릴경우 .join() 을 안해도 된다 */}
                                {selData.메뉴.map((v) => (
                                    <li>
                                        <a href="#">{v}</a>
                                    </li>
                ...
                {/* <!-- 2-2. 카테고리 페이지 컨텐츠영역 --> */}
                <div className="cat-cont-area fashion">
