...
// 정규식은 슬래쉬(/)사이에 쓰며
// \s는 스페이스 기호, g는 전역(global) 플래그(flag:기술용어로 처리표시 기호)다!

// 이름은(#mnm)일 경우 중간공백은 있어야 하므로
...
cv = cid=="mnm"? cv.trim(): groSpace(cv);
    ...
    *************************************/
   if(cv==""){//cv가 비었을때
    // (값이 안들어가고 다른데로 넘어가면 "필수입력")메시지 출력하기
     ...
     ****************************************/
    else if(cid == "mid"){//cid가 mid일때
        //검사결과
        console.log("아이디검사:",vReg(cv,cid));
        if(!vReg(cv,cid)){// 아이디 검사 false시
    ...
    /**************************************** 
     ...
     - 검사기준: 이메일 형식에 맞는지 여부
     ****************************************/
    else if(cid == "email1"){//cid가 email일때

    }///////////else if////////////
    /**************************************** 
     8. 모두 통과일 경우 메시지 지우기, (값이 들어갔을때 "필수입력")메시지 지우기
     ****************************************/     
    else{
