...
// [상품정보 생성을 위한 클래스]/////////
// 클래스는 객체 템플릿이다!
// 따라서 클래스는 생성자함수를 호출하면
// 객체 인스턴스가 생성된다!
// 인스턴스는: 메모리상에 실제 객체가 만들어진것을 의미!
class GetList {
  // 생성자 메서드
  constructor(idx, name, img, price) {
    ...
    this.img = img;
    this.price = price;
    //this는 GetList인스턴스 자신을 가리킨다!
  }
} /////////세미콜론은: 함수, 클래스, 각종제어문(if, for)일때 안한다!!!!
...
const vm = new Vue({
  ...
  data: {
    ...
    // 2-4. 상품정보 배열
    // 데이터를 클래스를 통해 생성하자
    itemData: [], //->배열 리터럴로 선언과 할당(있는그대로털어라)
  ...
  created() {
    ...
    for (let i = 1; i <= 18; i++) {
      ...
      // 2-2. 다양한 가격을 위해 4~20 사이에 난수곱
      let rdm2 = Math.ceil(Math.random() * 17) + 3;
      // 먼저1~17난수를 만들고 3을 더해 4~20만든다!
      console.log("랜덤1:", rdm1, "/랜덤2:", rdm2);
      // 2-3. 뷰ㅜ인스턴스의 itemData배열값 넣기
